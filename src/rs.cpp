...
int rs2_change_pose_origin_on_node(const rs2_sensor* sensor, const char* guid, double* effective_time, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(sensor);
    VALIDATE_NOT_NULL(guid);
...
int rs2_change_pose_origin_on_map_id(const rs2_sensor* sensor, const uint16_t mapId, double* effective_time, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(sensor);

    auto pose_snr = VALIDATE_INTERFACE(sensor->sensor, librealsense::pose_sensor_interface);
    return int(pose_snr->change_pose_origin(mapId, *effective_time));
}
